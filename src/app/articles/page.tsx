...
export default function ArticlesPage() {
  ...
  // Reusable fetch function that takes a starting index
  const fetchArticles = async (start: number) => {
    const end = start + ARTICLES_PER_PAGE;
    // The GROQ slice operator [start...end] limits the data we pull
    ...
    try {
      ...
      // If Sanity returns fewer articles than we asked for, we've hit the end!
      if (data.length < ARTICLES_PER_PAGE) {
        setHasMore(false);
      }

      // If it's the first load, set the articles. Otherwise, append the new ones.
      if (start === 0) {
        setArticles(data);
      } else {
  ...
  // Initial Load (Fetches the first 6)
  useEffect(() => {
    fetchArticles(0).finally(() => setIsLoadingInitial(false));
  }, []);

  // Load More Handler (Fetches the next 6 based on current array length)
  const handleLoadMore = async () => {
    setIsLoadingMore(true);
    await fetchArticles(articles.length);
    ...
    <>
      <main className="bg-slate-50 min-h-screen pb-10">
        ...
        <section className="max-w-7xl mx-auto px-6 mt-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {isLoadingInitial
              ? [1, 2, 3, 4, 5, 6].map((s) => (
                  <div
                    key={s}
                    className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 h-[400px] animate-pulse flex flex-col"
                  />
                ))
          ...
          {!isLoadingInitial && articles.length === 0 && (
