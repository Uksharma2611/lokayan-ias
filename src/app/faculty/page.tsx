...
export default function FacultyPage() {
  ...
  // Fetch Faculty from Sanity
  useEffect(() => {
    const fetchFaculty = async () => {
      // GROQ query to fetch faculty and get the actual image URL
        ...
        <section className="max-w-7xl mx-auto px-6">
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {[1, 2, 3].map((n) => (
                <div key={n} className="bg-white rounded-2xl p-8 border border-slate-100 animate-pulse flex flex-col items-center">
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                {/* FIXED: Changed to map over visibleFaculty instead of filteredFaculty */}
                        ...
                        {member.exams.map((exam) => (
                          <span 
                            ...
                            className={`text-[10px] font-bold px-2.5 py-1 rounded-sm uppercase tracking-wider ${
                              exam === 'UPSC' ? 'bg-blue-50 text-blue-700' : 'bg-red-50 text-[#ed1c24]'
                            }`}
                          >
                            {exam}
                          </span>
